...
<script id='tpl_shop_category_list' type='text/html'>
    ...
    <div class="fui-icon-group selecter">
        ...
        <?php  if($category_set['level']<=2) { ?>
            <%each recommend_children as child%>
                <a class="fui-icon-col external" href="<?php  echo mobileUrl('goods')?>&cate=<%child.id%>">
                    <div class="icon <?php  if(empty($set['style'])) { ?>radius<?php  } ?>"><img src="<%child.thumb%>" onerror="this.src='https://benla.oss-cn-beijing.aliyuncs.com/static/images/nopic100.jpg'; this.title='图片未找到.'"></div>
            ...
            <%each recommend_children as child%>
                <a class="fui-icon-col show" data-children="<%child.id%>" data-pid="recommend" data-src="<%child.advimg%>" data-href="<%child.advurl%>" href="javascript:">
                    <div class="icon <?php  if(empty($set['style'])) { ?>radius<?php  } ?>">
                        <img src="<%child.thumb%>" onerror="this.src='https://benla.oss-cn-beijing.aliyuncs.com/static/images/nopic100.jpg'; this.title='图片未找到.'">
                    </div>
                    <div class="text"><%child.name%></div>
                </a>
            <%/each%>
        ...
        <a class="fui-icon-col external" href="<?php  echo mobileUrl('goods')?>&cate=<%grandchild.id%>">
            <div class="icon <?php  if(empty($set['style'])) { ?>radius<?php  } ?>"><img src="<%grandchild.thumb%>" onerror="this.src='https://benla.oss-cn-beijing.aliyuncs.com/static/images/nopic100.jpg'; this.title='图片未找到.'"></div>
    ...
    <div class="fui-icon-group selecter">
        ...
        <a class="fui-icon-col external" href="<?php  echo mobileUrl('goods')?>&cate=<%cate.id%>">
            <div class="icon <?php  if(empty($set['style'])) { ?>radius<?php  } ?>"><img src="<%cate.advimg%>" onerror="this.src='https://benla.oss-cn-beijing.aliyuncs.com/static/images/nopic100.jpg'; this.title='图片未找到.'"></div>
