...
<div class="header-wrap">
    <header class="public-head-layout wrapper">
        ...
        <div class="nc-regist-now">
            ...
            <span>Hi，欢迎来                        <br>
                        还不是本站会员？立即                        <a title="" href="<?php  echo mobileUrl('pc.account.login')?>" class="register">
                            登陆</a>
                    </span>
        </div>
    </header>
        ...
        <div class="nc-register-layout">
            <div class="left">
                <div class="nc-register-mode">
                    <ul class="tabs-nav">
                        <li>
                            <a href="" class="tabulous_active"><?php  if(empty($type)) { ?>立即注册<?php  } else { ?>立即找回<?php  } ?><i></i></a>
                        </li>
                    </ul>
                    <div id="tabs_container" class="tabs-container" style="height: 387px;">
                        <div id="default" class="tabs-content" style="position: absolute; top: 0px;">
                            <form id="register_form" class="nc-login-form" method="post" action="">
                                <dl>
                                    <dt>手机号：</dt>
                                    <dd>
                                        <input type="tel" class="text" name="mobile" id="mobile" placeholder="您的手机号码" value="<?php  echo trim($_GPC['mobile'])?>" maxlength="11" />
                                    </dd>
                                </dl>
                                <div class="code-div">
                                    <dl>
                                        <dt>验证码：</dt>
                                        <dd>
                                            <input type="tel" class="text w120" name="verifycode" id="verifycode" placeholder="5位验证码"/>
                                        </dd>
                                    </dl>
                                    <span>
                                            <a  id="btnCode" style="display: block;">获取验证码</a>

                                        </span>
                                </div>
                                <div class="tiptext" id="sms_text">点击<span><a href="javascript:void(0);"><i class="icon-mobile-phone"></i>发送短信验证</a></span>，并将您手机短信所接收到的“5位动态码”输入到上方短信验证，再提交。
                                </div>

                                <dl>
                                    <dt>设置密码：</dt>
                                    <dd>
                                        <input type="password" class="text"  name="pwd" id="pwd" placeholder="6-20个大小写英文字母、符号或数字" value=""/>
                                    </dd>
                                </dl>
                                <dl>
                                    <dt>确认密码：</dt>
                                    <dd>
                                        <input type="password" class="text"  name="pwd1" id="pwd1" placeholder="请重新输入登录密码" value=""/>
                                    </dd>
                                </dl>


                                <div class="submit-div">
                                    <div class="submit" id="btnSubmit" style="text-align:center;line-height: 42px;"><?php  if(empty($type)) { ?>立即注册<?php  } else { ?>立即找回<?php  } ?></div>
                                    <?php  if(empty($type)) { ?>
                                    <a href="<?php  echo mobileUrl('pc.account.forget')?>" class="account-login-link  external ">忘记密码 </a>
                                    <?php  } else { ?>
                                    <a href="<?php  echo mobileUrl('pc.account.register')?>" class="account-login-link  external ">立即注册 </a>
                                    <?php  } ?>
...
<script language='javascript'>
    require(['../addons/ewei_shopv2/plugin/pc/biz/member/account.js'], function (modal) {
        modal.initRf({backurl:'<?php  echo $backurl;?>', type: <?php  echo intval($type)?>, endtime: <?php  echo intval($endtime)?>});
    });
</script>
</body></html>