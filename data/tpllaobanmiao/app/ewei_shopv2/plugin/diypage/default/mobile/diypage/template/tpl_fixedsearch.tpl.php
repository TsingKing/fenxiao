...
<?php  $diyitem = $diyitem_search?>
    <div class="diy-fixedsearch fixed" style="<?php  if(is_h5app()&&is_ios()) { ?> top: 3.2rem;<?php  } ?>">
        ...
        <form action="<?php  echo mobileUrl('goods', array('merchid'=>$page['merch']))?>" method="post" class="inner" id="fixedsearch">
            ...
            <div class="center <?php  echo $diyitem['params']['searchstyle'];?>">
                <input type="search" value="" placeholder="<?php  echo $diyitem['params']['placeholder'];?>" style="color: <?php  echo $diyitem['style']['searchtextcolor'];?>; opacity: <?php  echo $diyitem['style']['opacityinput'];?>; background: <?php  echo $diyitem['style']['searchbackground'];?>;" name="keywords" />
            </div>
            <?php  if(!empty($diyitem['params']['rightnav'])) { ?>
                <a class="rightnav"  <?php  if(empty($diyitem['params']['rightnavclick'])) { ?>href="<?php echo !empty($diyitem['params']['rightnavlink'])?$diyitem['params']['rightnavlink']:'javascript:;'?>" <?php  } else { ?>onclick="fixedsearch()"<?php  } ?> data-nocache="true">
                ...
                <?php  if(!empty($diyitem['params']['rightnav'])) { ?>
                <script>function fixedsearch() {$('#fixedsearch').submit();}</script>
                <?php  } ?>
            <?php  } ?>
        </form>
    </div>
