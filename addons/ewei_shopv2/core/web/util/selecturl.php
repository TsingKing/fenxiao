...
class Selecturl_EweiShopV2Page extends WebPage
{
	...
	public function check_perm($str)
	{
		global $_W;
		global $_GPC;
		...
		$permsInfo = pdo_fetch('SELECT perms2 FROM ' . tablename('ewei_shop_perm_user') . ' WHERE uid=:uid ', array(':uid' => $_W['user']['uid']));
		return in_array($str, explode(',', $permsInfo['perms2']));
	}
	...
	public function main()
	{
		...
		if ($this->check_perm('commission') && p('commission') && !($platform)) {
			$allUrls[] = array(
				...
				'list' => array(
					...
					array('name' => '我的伙伴', 'url' => mobileUrl('commission/down', NULL, $full), 'url_wxapp' => ''),
		...
		if ($this->check_perm('offic') && p('offic') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('article') && p('article') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('groups') && p('groups') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('mr') && p('mr') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('sns') && p('sns') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('sign') && p('sign') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('qa') && p('qa') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('bargain') && p('bargain') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('task') && p('task') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('creditshop') && p('creditshop') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('seckill') && p('seckill') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('newstore') && p('newstore') && !($platform)) {
			$allUrls[] = array(
		...
		if ($this->check_perm('globonus') && p('globonus') && !($platform)) {
			$allUrls[] = array(
