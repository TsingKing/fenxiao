	...
	public function getAllOrder(){
		...
		if(!empty($list)){
			...
			foreach($list as $key=>$val){
				...
				$address = pdo_fetch('select realname,mobile,province,city,area,street,address from '.tablename('ewei_shop_member_address').' where id=:id order by isdefault desc limit 1', array(':id'=>$val['addressid']));
				...
				$couponname = '';
				if(!empty($val['couponid'])){
					$couponname = pdo_fetch('SELECT c.couponname from '.tablename('ewei_shop_coupon').' as c right join '.tablename('ewei_shop_coupon_data').' as d on d.couponid=c.id where d.id='.$val[couponid]);
				}
				$data[$key]['remarksaler'] = $couponname['couponname'].$val['remarksaler'];
