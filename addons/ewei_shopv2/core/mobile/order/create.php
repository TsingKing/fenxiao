				...
				if (empty($bargain_id)) {
					foreach ($saleset['enoughs'] as $e ) {
						if ((floatval($e['enough']) <= $realprice - $seckill_payprice) && (0 < floatval($e['money']))) {
							...
							$saleset['showenough'] = true;
							$price = $realprice - $seckill_payprice;
							if( $price>1000 ){
								$money = $this->randomFloat(($price-1000)*0.06, $price*0.06);
								$saleset['enoughdeduct'] = $money;
								$realprice -= floatval($money);
							}else{
								$money = $this->randomFloat(0, $price*0.06);
								$saleset['enoughdeduct'] = $money;
								$realprice -= floatval($money);								
							}
						}
	...
	public function submit()
	{
		...
		foreach ($goods as $g ) {
			...
			if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
				...
				if (($data['buygroups'] != '') && !(empty($groupid))) {
