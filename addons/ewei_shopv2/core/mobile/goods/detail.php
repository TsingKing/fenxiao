...
class Detail_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		...
		/*猜你喜欢（推荐）*/
		$cates = pdo_get('ewei_shop_goods', array('id' => $id), array('cates'));
		if( !empty($cates) ){
			$goods_recommand = pdo_fetchall("SELECT id,title,thumb,marketprice FROM ".tablename('ewei_shop_goods')." WHERE cates=:cates and isrecommand=1 and status=1 and deleted=0 and id not in( " . $id . " ) ORDER BY RAND() LIMIT 3", array(':cates' => $cates['cates']));
			if($goods_recommand){	
				foreach($goods_recommand as $k=>$v){
					$goods_recommand[$k]['url'] = mobileUrl('goods/detail', array('id' => $v['id']));
					// $goods_recommand[$k]['thumb'] = $v['thumb'];
					$goods_recommand[$k]['title'] = mb_strlen($v['title'])>20 ? mb_substr($v['title'], 0, 20, 'utf-8').'...' : $v['title'] ; 
