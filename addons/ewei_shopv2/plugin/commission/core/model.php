...
if (!(class_exists('CommissionModel'))) {
	class CommissionModel extends PluginModel
	{
		public function getSet($uniacid = 0)
		{
			...
			$set['texts'] = array('agent' => (empty($set['texts']['agent']) ? '分销商' : $set['texts']['agent']), 'shop' => (empty($set['texts']['shop']) ? '小店' : $set['texts']['shop']), 'myshop' => (empty($set['texts']['myshop']) ? '我的小店' : $set['texts']['myshop']), 'center' => (empty($set['texts']['center']) ? '分销中心' : $set['texts']['center']), 'become' => (empty($set['texts']['become']) ? '成为分销商' : $set['texts']['become']), 'withdraw' => (empty($set['texts']['withdraw']) ? '提现' : $set['texts']['withdraw']), 'commission' => (empty($set['texts']['commission']) ? '佣金' : $set['texts']['commission']), 'commission1' => (empty($set['texts']['commission1']) ? '分销佣金' : $set['texts']['commission1']), 'commission_total' => (empty($set['texts']['commission_total']) ? '累计佣金' : $set['texts']['commission_total']), 'commission_ok' => (empty($set['texts']['commission_ok']) ? '可提现佣金' : $set['texts']['commission_ok']), 'commission_apply' => (empty($set['texts']['commission_apply']) ? '已申请佣金' : $set['texts']['commission_apply']), 'commission_check' => (empty($set['texts']['commission_check']) ? '待打款佣金' : $set['texts']['commission_check']), 'commission_lock' => (empty($set['texts']['commission_lock']) ? '未结算佣金' : $set['texts']['commission_lock']), 'commission_detail' => (empty($set['texts']['commission_detail']) ? '提现明细' : (($set['texts']['commission_detail'] == '佣金明细' ? '提现明细' : $set['texts']['commission_detail']))), 'commission_pay' => (empty($set['texts']['commission_pay']) ? '成功提现佣金' : $set['texts']['commission_pay']), 'commission_wait' => (empty($set['texts']['commission_wait']) ? '待收货佣金' : $set['texts']['commission_wait']), 'commission_fail' => (empty($set['texts']['commission_fail']) ? '无效佣金' : $set['texts']['commission_fail']), 'commission_charge' => (empty($set['texts']['commission_charge']) ? '扣除提现手续费' : $set['texts']['commission_charge']), 'order' => (empty($set['texts']['order']) ? '分销订单' : $set['texts']['order']), 'c1' => (empty($set['texts']['c1']) ? '一级' : $set['texts']['c1']), 'c2' => (empty($set['texts']['c2']) ? '二级' : $set['texts']['c2']), 'c3' => (empty($set['texts']['c3']) ? '三级' : $set['texts']['c3']), 'mydown' => (empty($set['texts']['mydown']) ? '我的伙伴' : $set['texts']['mydown']), 'down' => (empty($set['texts']['down']) ? '伙伴' : $set['texts']['down']), 'up' => (empty($set['texts']['up']) ? '推荐人' : $set['texts']['up']), 'yuan' => (empty($set['texts']['yuan']) ? '元' : $set['texts']['yuan']), 'icode' => (empty($set['texts']['icode']) ? '邀请码' : $set['texts']['icode']));
			return $set;
		}
		...
		public function calculate($orderid = 0, $update = true, $order_agentid = NULL)
		{
			...
			$order = pdo_fetch('select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid,couponid from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
			...
			if (0 < $set['level']) {
				foreach ($goods as &$cinfo ) {
					...
					if (!(empty($seckill_goods))) {
						...
						$member_commission = !empty($cinfo['hasoriginal']) || ( empty($cinfo['hasoriginal']) && $cinfo['goodsprice']!=$cinfo['realprice'] );
						// 不返佣******优惠券不返佣*******会员价格不返佣
						// empty($order['couponid']) 2019-04-09 使用优惠券不享受返佣
						// if (empty($cinfo['nocommission']) && empty($order['couponid']) && $member_commission) {
						if (empty($cinfo['nocommission']) && $member_commission) {
							$hascommission = true;

							if ($cinfo['hascommission'] == 1) {
								
								//2018-11-15 会员价是否有佣金判定
								// if( ( ( !empty( $cinfo['hasoriginal'] ) && $cinfo['goodsprice'] == $cinfo['realprice'] ) || empty( $cinfo['hasoriginal'] ) ) ){
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '')
		{
			...
			if (($message_type == TM_COMMISSION_AGENT_NEW) && empty($usernotice['commission_agent_new'])) {
				if ($tm['is_advanced']) {
					...
					$msg = array(
						'keyword1' => array('value' => (!(empty($tm['commission_agent_newtitle'])) ? $tm['commission_agent_newtitle'] : '新增伙伴通知'), 'color' => '#73a68d'),
				...
				if ($tm['is_advanced']) {
					...
					$msg = array(
						'keyword1' => array('value' => (!(empty($tm['commission_order_paytitle'])) ? $tm['commission_order_paytitle'] : '伙伴付款通知')),
				...
				if ($tm['is_advanced']) {
					...
					$message = str_replace('[商品详情]', $data['goods'], $message);
					$msg = array(
						'keyword1' => array('value' => (!(empty($tm['commission_order_finishtitle'])) ? $tm['commission_order_finishtitle'] : '伙伴确认收货通知'), 'color' => '#73a68d'),
						'keyword2' => array('value' => $message, 'color' => '#73a68d')
						);
			...
			if ($tm['is_advanced'] && (($tag == 'commission_agent_new') || ($tag == 'commission_order_pay') || ($tag == 'commission_order_finish'))) {
				...
				$datas[] = array('name' => '伙伴层级', 'value' => $set['texts']['c1']);
				...
				foreach ($res as $key => $value ) {
					if ($key == 0) {
						...
						$datas[] = array('name' => '伙伴层级', 'value' => $set['texts']['c2']);
						...
						if ($tag == 'commission_order_pay') {
							$text = '您的二级伙伴' . $data['nickname'] . '已付款！';
						}
						 else if ($tag == 'commission_order_finish') {
							$text = '您的二级伙伴' . $data['nickname'] . '已确认收货！';
						}
					...
					if ($key == 1) {
						...
						$datas[] = array('name' => '伙伴层级', 'value' => $set['texts']['c3']);
						...
						if ($tag == 'commission_order_pay') {
							$text = '您的三级伙伴' . $data['nickname'] . '已付款！';
						}
						 else if ($tag == 'commission_order_finish') {
							$text = '您的三级伙伴' . $data['nickname'] . '已确认收货！';
						}
		...
		protected function sendMore($touser, $tm, $tag, $msg, $datas)
		{
			...
			$set = $this->getSet();
			$msgbk = $msg;
			$msgbk['keyword2']['value'] = str_replace('[伙伴层级]', $set['texts']['c1'], $msgbk['keyword2']['value']);
			$msgbk['keyword2']['value'] = str_replace('[伙伴]', $set['texts']['down'], $msgbk['keyword2']['value']);
			if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) {
				if ($datas['isagent']) {
			...
			foreach ($res as $key => $value ) {
				...
				if ($key == 0) {
					$msgbk['keyword2']['value'] = str_replace('[伙伴层级]', $set['texts']['c2'], $msgbk['keyword2']['value']);
					$msgbk['keyword2']['value'] = str_replace('[伙伴]', $set['texts']['c2'] . $set['texts']['down'], $msgbk['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) {
						if ($datas['isagent']) {
				...
				if ($key == 1) {
					$msgbk['keyword2']['value'] = str_replace('[伙伴层级]', $set['texts']['c3'], $msgbk['keyword2']['value']);
					$msgbk['keyword2']['value'] = str_replace('[伙伴]', $set['texts']['c3'] . $set['texts']['down'], $msgbk['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) {
						if ($datas['isagent']) {
		...
		protected function sendMoreAdvanced($touser, $tm, $tag, $msg, $url, $datas)
		{
			...
			$msgbk = $this->replaceArray($msgbk, '[伙伴层级]', $set['texts']['c1']);
			...
			foreach ($res as $key => $value ) {
				if ($key == 0) {
					...
					$msgbk = $this->replaceArray($msgbk, '[伙伴层级]', $set['texts']['c2']);
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) {
				...
				if ($key == 1) {
					...
					$msgbk = $this->replaceArray($msgbk, '[伙伴层级]', $set['texts']['c3']);
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) {
