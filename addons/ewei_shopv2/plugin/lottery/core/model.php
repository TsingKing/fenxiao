...
class LotteryModel extends PluginModel
{
			...
			if ($type == 1) {
					...
					if (($value['task_data']['pay_type'] == 0) || ($data['paytype'] == $value['task_data']['pay_type'])) {
						if ($value['task_data']['pay_money'] <= $data['money']) {
							if ($join_info['money'] < $value['task_data']['pay_money']) {
								...
								$join_info['times'] = $value['task_data']['pay_num'];
								$join_info['num'] = intval(floor($data['money']/$value['task_data']['pay_money'])) * $value['task_data']['pay_num'];//每满多少送几次
				...
				if (!empty($join_info['lottery_id'])) {
					...
					while ($i <= $join_info['num']) {
						$join_data = array('uniacid' => $_W['uniacid'], 'join_user' => $openid, 'lottery_id' => $join_info['lottery_id'], 'lottery_num' => 1, 'lottery_tag' => '消费满' . $join_info['money'] . '元,赠' . $join_info['times'] . '次', 'addtime' => time());
	...
	public function lottery_complain($reward)
	{
		...
		if (isset($reward['goods'])) {
			...
			return '特惠商品:' . $title . $total . '个';
		}

		if (isset($reward['coupon'])) {
			foreach ($reward['coupon'] as $k => $v) {
				$name = $v['couponname'];
				break;
			}
			return '优惠券:' . $name . $reward['coupon']['total'] . '张';
		}
	}
